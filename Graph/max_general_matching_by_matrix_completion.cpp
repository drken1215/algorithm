...
//
//   ACPC 2018 G - Combine Two Elements (AOJ 3032)
...
void AOJ_3032() {
    int N, A, B, unit = 0;
    cin >> N >> A >> B;
    vector<int> a(N), b(N), used(N, false);
    for (int i = 0; i < N; i++) {
        cin >> a[i] >> b[i];
        int dis = abs(a[i] - b[i]);
        if (dis <= A || (B <= dis && dis <= A * 2)) unit++, used[i] = true;
    }
    vector G(N, vector(N, false));
    for (int i = 0; i < N; i++) for (int j = i+1; j < N; j++) {
        if (used[i] || used[j]) continue;
        int dis = abs((a[i] + a[j]) - (b[i] + b[j]));
        if (dis <= A || (B <= dis && dis <= A * 2)) G[i][j] = G[j][i] = true;
    }
    using mint = Fp<>;
    GeneralMatching<mint> gm(G);
    int match = gm.solve().size();
    int res = unit + match;
    cout << res << endl;
}
