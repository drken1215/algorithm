...
// Yosupo Library Checker - Convolution
void Yosupo_Convolution_cincout() {
...
// Yosupo Library Checker - Convolution (Fast IO ver.)
void Yosupo_Convolution_FastIO() {
    const int MOD = 998244353;
    using mint = Fp<MOD>;
    ...
    for (int i = 0; i < M; i++) Read(b[i].val);
    
    auto c = convolution_ntt(a, b);
    for (int i = 0; i < c.size(); i++) Write(c[i].val), Write(' ');
    Write('\n');
}


int main() {
    //Yosupo_Convolution_cincout();
    Yosupo_Convolution_FastIO();
}