//
// S1, ..., SN の順列を concat してできる文字列のうち、辞書順最小のものを求める
//
// verified:
...
vector<int> smallest_string_concat(const vector<string> &S) {
    int N = (int)S.size();
    vector<int> res(N, 0), left(N, 0);
    string T = "";
    for (int i = 0; i < N; i++) res[i] = i, left[i] = T.size(), T += S[i];
    SuffixArray sa(T);

    auto cmp = [&](int i, int j) -> bool {
        if (sa.get_lcp(left[i], left[j]) < min(S[i].size(), S[j].size())) {
            return sa.rank[left[i]] < sa.rank[left[j]];
        } else if (S[i].size() >= S[j].size()) {
            int diff = (int)S[i].size() - (int)S[j].size();
            if (sa.get_lcp(left[i], left[i]+S[j].size()) < diff) {
                return sa.rank[left[i]+S[j].size()] < sa.rank[left[i]];
            } else {
                return sa.rank[left[j]] < sa.rank[left[i]+diff];
            }
        } else {
            int diff = (int)S[j].size() - (int)S[i].size();
            if (sa.get_lcp(left[j], left[j]+S[i].size()) < diff) {
                return sa.rank[left[j]] < sa.rank[left[j]+S[i].size()];
            } else {
                return sa.rank[left[j]+diff] < sa.rank[left[i]];
            }
        }
    };
...
// Educational Codeforces Round 9 C. The Smallest String Concatenation
void Codeforces_The_Smallest_String_Concatenation() {
    int N;
    cin >> N;
    vector<string> S(N);
    for (int i = 0; i < N; i++) cin >> S[i];
    auto ids = smallest_string_concat(S);
    string res = "";
    for (int i = 0; i < N; i++) res += S[ids[i]];
    cout << res << endl;
}


int main() {
    Codeforces_The_Smallest_String_Concatenation();
}