 ...
 */
 ...
 // JOI 予選 2020 二次予選 E - じゃんけん式
 void JOI_2020_2YOSEN_E() {
    const int R = 0, S = 1, P = 2;
    const int MOD = 1000000007;
    using mint = Fp<MOD>;
    struct Node {
        vector<mint> dat;
        Node(const vector<mint> &dat) : dat(dat) {}
        Node(mint r = 0, mint s = 0, mint p = 0) : dat({r, s, p}) {}
    };
     ...
     char A;
     cin >> N >> E >> A;
     Parser<Node> ps(operators, operation, get_leaf);
     auto res = ps.eval(E, false);
 
     if (A == 'R') cout << res.dat[0] << endl;
     else if (A == 'S') cout << res.dat[1] << endl;
     else cout << res.dat[2] << endl;
 }
