...
//
//　　AOJ 1460 - The Farthest Point
//　　　https://onlinejudge.u-aizu.ac.jp/problems/1460
//
...
void AOJ_1460() {
    ...
    cin >> a[0] >> a[1] >> a[2];
    sort(a.begin(), a.end());
    DD A = a[0], B = a[1], C = a[2];

    if (B > C * (C * 2 - A) / (C * 2 + A)) {
        cout << sqrt(A * A + B * B + C * C + A * B * 2) << endl;
    } else {
        Point<DD> p(0, 0);
        Point<DD> q(A, A + C * 2);
        Point<DD> r(A + B + C, A + B + C);
        Circle<DD> circle = CircumscribedCircle(p, q, r);
        cout << circle.r << endl;
    }
}
