//
// 多項式マージテク：
//     次数の総和が D である多項式の総積 in O(D (log D)^2)
//
// verified:
//   Yosupo Library Checker - Product of Polynomial Sequence
...
// Yosupo Library Checker - Product of Polynomial Sequence
void Yosupo_Product_of_Polynomial_Sequence() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        cin >> D;
        FPS<mint> f(D + 1);
        for (int j = 0; j <= D; j++) cin >> f[j];
        fs.emplace_back(f);
    }
    auto res = all_product(fs);
    cout << res << endl;
}
