//
// 多項式の基底変換：Monomial 基底 → Newton 基底 (O(N(log N)^2))
//
// verified:
//   Yosupo Library Checker - Conversion from Monomial Basis to Newton Basis
...
// polynomial basis conversion: monomial basis -> Newton basis
// inupt: coefficients of f(x), and p[0], p[1], ..., p[N-1]
// output: b[0], b[1], ..., b[N-1] s.t. f(x) = sum_{i=0}^{N-1} b[i] prod_{j=0}^{i-1} (x - p[i])
template<class mint> vector<mint> conversion_basis_from_monomial_to_newton(
const FPS<mint> &f, const vector<mint> &p) {
    int N = (int)f.size();
    int B = 1;
    while (B < N) B <<= 1;
    vector<FPS<mint>> mod(B * 2, FPS<mint>{1});
    for (int i = 0; i < N; i++) mod[i + B] = FPS<mint>{-p[i], 1};
    for (int i = B - 1; i > 0; i--) mod[i] = mod[i * 2] * mod[i * 2 + 1];
    vector<mint> res(N);
    auto rec = [&](auto &&rec, int id, int l, int r, FPS<mint> g) -> void {
        if (N <= l) return;
        if (l + 1 == r) {
            res[l] = g.eval(0);
            return;
        ...
        int m = (l + r) / 2;
        auto quo = g / mod[id * 2], rem = g - quo * mod[id * 2];
        rec(rec, id * 2, l, m, rem);
        rec(rec, id * 2 + 1, m, r, quo);
    };
    rec(rec, 1, 0, B, f);
    return res;
}
...
// Yosupo Library Checker - Conversion from Monomial Basis to Newton Basis
void Yosupo_Conversion_from_Monomial_to_Newton() {
    ...
    FPS<mint> f(N);
    vector<mint> p(N);
    for (int i = 0; i < N; i++) Read(f[i].val);
    for (int i = 0; i < N; i++) Read(p[i].val);
    auto res = conversion_basis_from_monomial_to_newton(f, p);
...
int main() {
    Yosupo_Conversion_from_Monomial_to_Newton();
}