//
// Multipoint Evaluation (等比数列のとき) (by chirp z-transform, O(N log N))
//
// verified:
//   Yosupo Library Checker - Multipoint Evaluation (Geometric Sequence)
...
// Yosupo Library Checker - Yosupo Library Checker - Multipoint Evaluation (Geometric Sequence)
void Yosupo_Multipoint_Evaluation_Geometric_Sequence() {
    FastRead Read; FastWrite Write;
    using mint = Fp<>;
    int N, M;
    mint a, r;
    Read(N, M, a.val, r.val);
    FPS<mint> f(N);
    for (int i = 0; i < N; i++) Read(f[i].val);
    auto res = multipoint_eval(f, a, r, M);
...
int main() {
    Yosupo_Multipoint_Evaluation_Geometric_Sequence();
}